...
export default function TreeTabHeader({ activeTab, setActiveTab, onDbTabContextMenu, onBrokerTabContextMenu, onLogTabContextMenu, onRefreshLog, isRefreshing }) {
  const CM = useCM();
    ...
    <div className="flex items-center gap-0.5 px-2 py-2 border-b border-slate-100 dark:border-white/5 bg-white dark:bg-background-dark shrink-0">
      ...
      {activeTab === 'log' && (
        <button
          ...
          className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-md text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-all duration-150 flex items-center justify-center shrink-0"
          title={CM.refreshLog || CM.refresh}
        >
          <Icon
            name="refresh"
            size="sm"
            className={isRefreshing ? 'animate-spin text-amber-500' : ''}
          />
        </button>
      )}
    </div>
