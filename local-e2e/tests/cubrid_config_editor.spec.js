...
// Save writes straight to the host's real cubrid.conf with no diff/preview
// step — never click it for real. Only open, edit, and undo.
test.describe('Cubrid Config Editor', () => {
  let hostUid;
  ...
  test('Edit Cubrid Config를 열면 설정 내용이 표시되고, 수정하면 dirty 표시가 되며, Undo로 되돌릴 수 있다', async ({ page }) => {
    ...
    const textarea = page.getByTestId('cubrid-config-textarea');
    ...
    const saveBtn = page.getByTestId('cubrid-config-save-btn');
    const undoBtn = page.getByTestId('cubrid-config-undo-btn');
